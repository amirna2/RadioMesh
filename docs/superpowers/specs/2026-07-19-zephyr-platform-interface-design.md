...
**Zephyr TX model (assessed 2026-07-19 against both in-tree backends; the build uses the native one — `CONFIG_LORA_MODULE_BACKEND_NATIVE=y` since M1, `loramac-node` is deprecated):** both backends are fully interrupt-driven at every layer (DIO1 → workqueue → IRQ dispatch); "blocking" `lora_send` is a convenience wrapper that sleeps the caller on a `k_poll_signal` raised from the interrupt path. To preserve Arduino behavior (`sendPacket` = RadioLib `startTransmit`: non-blocking, completion via flag), `ZephyrLoraRadio::sendPacket` uses `lora_send_async` + `k_poll_signal`, and `checkAndClearTxFlag` = `k_poll_signal_check` — the signal IS the flag, polled by `Device::run()` exactly as on Arduino; app rx/tx callbacks keep firing from the run loop. Constraints encoded (verified in `drivers/lora/native/sx126x/sx126x.c`): async RX must be cancelled before TX (state machine enforces modem exclusivity → `-EBUSY`); the native backend raises the signal on BOTH TxDone (result 0) and chip-level TX timeout (result `-ETIMEDOUT`, 10 s), which `checkAndClearTxFlag` maps to `RM_E_RADIO_TX_TIMEOUT` — a 2×`lora_airtime()` deadline remains as a backstop because the deprecated loramac-node backend raises nothing on timeout; PHY-CRC-corrupt frames are dropped and RX re-armed inside the driver (explicit in the native source: "only report valid packets"), so `RM_E_RADIO_CRC_MISMATCH` never surfaces on Zephyr — the protocol's own CRC32 + MIC still validate end-to-end.

### Seam 3 — Storage: `IByteStorage` is already the contract

No interface change. `EEPROMStorage` (layout, defrag, `EEPROM` API) is Arduino-port internals and stays untouched. Zephyr: `ports/zephyr/hal/{inc,src}/storage/ZephyrNVSStorage.{h,cpp}` implements `IByteStorage` natively over NVS (string key → NVS id mapping; NVS writes are atomic and immediately persistent, so `write` writes directly and `commit`/`defragment` are documented no-ops; header `<zephyr/kvss/nvs.h>`, Kconfig `CONFIG_NVS`; `storage_partition` verified present in the XIAO's `espressif/partitions_0x0_amp_4M.dtsi` — `partition@3b0000`, 192 KiB, no overlay change needed). `Device`'s member becomes `IByteStorage*`; storage sizing/config moves into the platform factory (`ByteStorageParams` is an impl detail of each port). `DeviceStorage.h` swaps its `EEPROMStorage.h` include for `IByteStorage.h` (it already holds `IByteStorage*`).
