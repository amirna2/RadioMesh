...
**Zephyr TX model (assessed 2026-07-19 against `drivers/lora/loramac-node/sx12xx_common.c`):** the Zephyr driver is fully interrupt-driven at every layer (DIO1 → workqueue → LoRaMac-node `IrqProcess`); "blocking" `lora_send` is a convenience wrapper that sleeps the caller on a `k_poll_signal` raised by the TxDone interrupt. To preserve Arduino behavior (`sendPacket` = RadioLib `startTransmit`: non-blocking, completion via flag), `ZephyrLoraRadio::sendPacket` uses `lora_send_async` + `k_poll_signal`, and `checkAndClearTxFlag` = `k_poll_signal_check` — the signal IS the flag, polled by `Device::run()` exactly as on Arduino; app rx/tx callbacks keep firing from the run loop. Constraints encoded: async RX must be cancelled before TX (the driver enforces modem exclusivity → `-EBUSY`); a TX timeout raises no signal (`sx12xx_ev_tx_timed_out` only releases the modem), so a 2×`lora_airtime()` deadline in `checkAndClearTxFlag` supplies `RM_E_RADIO_TX_TIMEOUT` and force-releases the modem; PHY-CRC-corrupt frames are dropped and RX re-armed inside the driver, so `RM_E_RADIO_CRC_MISMATCH` never surfaces on Zephyr — the protocol's own CRC32 + MIC still validate end-to-end.
