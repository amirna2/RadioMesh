...
**Zephyr TX model (assessed 2026-07-19 against the in-tree native driver, `CONFIG_LORA_MODULE_BACKEND_NATIVE=y` since M1):** the driver is fully interrupt-driven at every layer (DIO1 → workqueue → IRQ dispatch); "blocking" `lora_send` is a convenience wrapper that sleeps the caller on a `k_poll_signal` raised from the interrupt path. To preserve Arduino behavior (`sendPacket` = RadioLib `startTransmit`: non-blocking, completion via flag), `ZephyrLoraRadio::sendPacket` uses `lora_send_async` + `k_poll_signal`, and `checkAndClearTxFlag` = `k_poll_signal_check` — the signal IS the flag, polled by `Device::run()` exactly as on Arduino; app rx/tx callbacks keep firing from the run loop. Constraints encoded (verified in `drivers/lora/native/sx126x/sx126x.c`): async RX must be cancelled before TX (state machine enforces modem exclusivity → `-EBUSY`); the driver raises the signal on BOTH TxDone (result 0) and chip-level TX timeout (result `-ETIMEDOUT`, 10 s), which `checkAndClearTxFlag` maps to `RM_E_RADIO_TX_TIMEOUT` — a 2×`lora_airtime()` deadline backstops a lost completion; PHY-CRC-corrupt frames are dropped and RX re-armed inside the driver (explicit in the source: "only report valid packets"), so `RM_E_RADIO_CRC_MISMATCH` never surfaces on Zephyr — the protocol's own CRC32 + MIC still validate end-to-end.
